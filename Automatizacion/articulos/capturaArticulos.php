...
$sql = "SELECT id_product, id_product_attribute FROM prstshp_product_attribute WHERE reference = '{$referencia}'";
...
if ($result->num_rows > 0) {
	...
	$sql = "SELECT id_product FROM prstshp_product WHERE reference LIKE '{$referencia}%'";
	$result = $conn->query($sql);
	if ($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
			$id_product =$row[id_product];
