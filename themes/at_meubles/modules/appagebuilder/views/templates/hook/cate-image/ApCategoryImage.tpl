...
{foreach $data as $category}
	...
	<li class="cate_{$category.id_category|intval}">
		<a href="{$link->getCategoryLink($category.id_category, $category.link_rewrite)|escape:'html':'UTF-8'}">
			...
			<div class="nbproducts inner">
				<div class="text">{$category.name|escape:'html':'UTF-8'}</div>
				<span class="num">
					<span id="leo-cat-{$category.id_category}" class="leo-qty leo-cat-{$category.id_category}" data-str="{l s='item(s)' d='Shop.Theme.Catalog'}"></span> 
				</span>
			</div>
		</a>
	</li>
